...
namespace Infrastructure.Identity.Services
{
    public class IdentityService : IIdentityService
    {
        ...
        public async Task<Result<TokenResponse>> RefreshTokenAsync(string token, string ipAddress)
        {
            var user = _context.Users.SingleOrDefault(u => u.RefreshTokens.Any(t => t.Token == token));
            Throw.Exception.IfNull(user, nameof(user), $"Token did not match any users.");
            var refreshToken = user.RefreshTokens.Single(x => x.Token == token);
            Throw.Exception.IfFalse(refreshToken.IsActive, $"Token Not Active.");
        ...
        public async Task<Result<string>> RegisterAsync(RegisterRequest request, string origin)
        {
            ...
            if (userWithSameEmail == null)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    await _mailService.SendAsync(new MailRequest() { From = "johinerjmartinez@gmail.com", To = user.Email, Body = $"Please confirm your account by <a href='{verificationUri}'>clicking here</a>.", Subject = "Confirm Registration" });
